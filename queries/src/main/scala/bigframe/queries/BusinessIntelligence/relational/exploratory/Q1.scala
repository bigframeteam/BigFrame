 ...
 *
 */
class Q1(basePath : BaseTablePath) extends Query with HiveRunnable with SharkRunnable with HadoopRunnable{

	protected val itemHDFSPath = basePath.relational_path + "/item"
	protected val web_salesHDFSPath = basePath.relational_path + "/web_sales"
	protected val catalog_salesHDFSPath = basePath.relational_path + "/catalog_sales"
	protected val store_salesHDFSPath = basePath.relational_path + "/store_sales"
	protected val promotionHDFSPath = basePath.relational_path + "/promotion"
	protected val customerHDFSPath =  basePath.relational_path + "/customer"
	 ...
	 */
	override def run(mapred_config: Configuration): Unit = {
		try {
			val reportsales = new ReportSalesHadoop(basePath.relational_path)
			reportsales.run(mapred_config)
			
		} catch {
			case ioe: IOException => ioe.printStackTrace()
			case inte: InterruptedException => inte.printStackTrace()
			case clznotFe: ClassNotFoundException => clznotFe.printStackTrace()
		}
