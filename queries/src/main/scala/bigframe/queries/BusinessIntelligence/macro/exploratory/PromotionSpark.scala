...
import bigframe.queries.BusinessIntelligence.relational.exploratory.ReportSalesSpark
import bigframe.queries.BusinessIntelligence.nested.exploratory.SenAnalyzeSpark

class PromotionSpark(basePath : BaseTablePath) extends SparkRunnable {
	...
	def run(spark_context: SparkContext): RDD[_]  = {
	  val tpcdsExecutor: ReportSalesSpark = new ReportSalesSpark(basePath)
	  val textExecutor: SenAnalyzeSpark = new SenAnalyzeSpark(basePath)
