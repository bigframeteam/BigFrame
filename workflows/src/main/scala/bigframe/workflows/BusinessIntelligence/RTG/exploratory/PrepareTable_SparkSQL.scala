...
class PrepareTable_SparkSQL(basePath: BaseTablePath) {
  /**
   * ORC file is not used
   */
  def prepareTableImpl1(hc: HiveContext): Unit = {
	System.out.println("Preparing tables sparksql");
		...
		val twitter_graphHDFSPath = basePath.graph_path
    ...
    hc.hql(dropWebSales)
    hc.hql(dropCatalogSales)
    hc.hql(dropStoreSales)
    hc.hql(dropItem)
    hc.hql(dropPromot)
    hc.hql(dropCust)
    hc.hql(dropDateDim)
...
/*
hc.hql("ADD JAR lib/json-serde-1.1.9.3-SNAPSHOT-jar-with-dependencies.jar")
  */  
    ...
    val createGraph = "create external table twitter_graph" +
      ...
      " )" +
      " row format delimited fields terminated by \'|\'" +
      " location \'" + twitter_graphHDFSPath + "\'"
...
*/
    ...
    hc.hql(createTweets)
    hc.hql(createWebSales)
    hc.hql(createCatalogSales)
    hc.hql(createStoreSales)
    hc.hql(createItem)
    hc.hql(createCust)
    hc.hql(createPromot)
    hc.hql(createDateDim)
    hc.hql(createGraph)
...
hc.hql("create temporary function sentiment as \'bigframe.workflows.util.SenExtractorHive\'")
hc.hql("create temporary function isWithinDate as \'bigframe.workflows.util.WithinDateHive\'")
hc.hql("set hive.auto.convert.join=false")
...
/**
	 * ORC file is used
	 */
	def prepareTableImpl2(hc: HiveContext): Unit = {
		val itemHDFSPath = basePath.relational_path + "/item"
		val web_salesHDFSPath = basePath.relational_path + "/web_sales"
		val catalog_salesHDFSPath = basePath.relational_path + "/catalog_sales"
		val store_salesHDFSPath = basePath.relational_path + "/store_sales"
		...
		hc.hql(dropWebSalesTmp)
		hc.hql(dropCatalogSalesTmp)
		hc.hql(dropStoreSalesTmp)
		hc.hql(dropItemTmp)
		hc.hql(dropPromotTmp)
		hc.hql(dropCustTmp)
		hc.hql(dropDateDimTmp)
		hc.hql(dropGraphTmp)
		hc.hql(dropTweetsTmp)
		...
		hc.hql(createWebSalesTmp)
		hc.hql(createCatalogSalesTmp)
		hc.hql(createStoreSalesTmp)
		hc.hql(createItemTmp)
		hc.hql(createCustTmp)
		hc.hql(createPromotTmp)
		hc.hql(createDateDimTmp)
		hc.hql(createGraphTmp)
		hc.hql(createTweetsTmp)
